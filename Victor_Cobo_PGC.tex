...
O conceito de contínuo IoT-Nuvem (\textit{IoT-Cloud Continuum}), frequentemente denominado de \textit{IoTinuum}, representa a integração e a orquestração unificada de recursos computacionais distribuídos ao longo de toda a infraestrutura física e de rede, estendendo-se desde os dispositivos de borda mais básicos (Things/Sensores) até os datacenters de nuvem de alta capacidade, passando pelas camadas intermediárias da névoa. 

Em vez de tratar a névoa e a nuvem como silos de computação isolados, o \textit{IoTinuum} propõe um ambiente de execução contínuo no qual as tarefas de processamento de dados, filtragem e tomada de decisão são dinamicamente alocadas e executadas no ponto mais adequado da rede. No monitoramento agrícola inteligente, o contínuo permite balancear o consumo de energia dos nós sensores de campo com a largura de banda disponível nos gateways locais de névoa e a capacidade de processamento analítico histórico na nuvem, otimizando o fluxo de dados de ponta a ponta \cite{kamienski2017iotinuum}.

\section{Rust e WebAssembly (WASM)}

\subsection{Linguagem Rust}

Rust é uma linguagem de programação focada em segurança, velocidade e concorrência \cite{matsakis2014rust}. Seu sistema de tipos e o conceito de posse (\textit{ownership}) com checagem de tempo de vida (\textit{borrow checker}) eliminam erros comuns de gerência de memória em tempo de compilação, sem depender de coleta de lixo. Isso garante um consumo de memória previsível e baixo, ideal para os dispositivos de borda.

\subsection{WebAssembly (WASM) e WASI}

WebAssembly é um formato de instrução binária projetado para ser executado de forma portável em uma máquina virtual baseada em pilha \cite{haas2017wasm}. Embora originalmente concebido para navegadores web, o WASM expandiu-se para o lado do servidor por meio da \textit{WebAssembly System Interface} (WASI), que padroniza o acesso a recursos do sistema operacional (como arquivos, rede e relógio). O bytecode WASM é executado sob isolamento completo (\textit{sandbox}), protegendo o gateway hospedeiro contra códigos maliciosos ou falhas de execução.

\subsection{WasmEdge Runtime}

WasmEdge é um interpretador e compilador JIT/AOT de WebAssembly de alto desempenho otimizado para cenários de computação de borda, névoa e microsserviços \cite{wasmedge2023}. Desenvolvido sob a tutela da Cloud Native Computing Foundation (CNCF), ele suporta a especificação WASI e oferece extensões para acelerar inferência de redes neurais, tornando-o o runtime ideal para executar pipelines de processamento estocástico e classificação local.
...
Este projeto baseia-se diretamente na metodologia proposta por Ribeiro Junior et al. \cite{ribeiro2020nearest}, intitulada \textit{"A Nearest Neighbors based Data Filter for Fog Computing in IoT Smart Agriculture"}. Aquele artigo estabeleceu as bases teóricas de um filtro de dados inteligente instalado em gateways de névoa locais.
...
Embora cientificamente elegante, o projeto original de Ribeiro Junior et al. \cite{ribeiro2020nearest} foi concebido de forma puramente teórica e avaliado localmente via scripts de simulação offline (MATLAB/Python). O presente trabalho promove os seguintes avanços metodológicos e de engenharia de software:
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{esquema_logico.png} 
...
\begin{itemize}
    \item \textbf{Lote 6} (representando 30 segundos de dados físicos);
    \item \textbf{Lote 60} (5 minutos);
    \item \textbf{Lote 720} (1 hora);
    \item \textbf{Lote 4.320} (6 horas);
    \item \textbf{Lote 20.000} (aproximadamente 14 horas de dados contínuos).
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{analise-experimentos/densidade_global_comparacao.png}
